...
2. **Schema endpoints with `stream=true`** — the request schema carries the `stream` flag (chat, completion, transcription, speech, video generation). OpenAPI lists that field only when the endpoint handler is classified as streaming by `is_streaming_endpoint` (generator function, async generator, or return annotation containing `Iterator` / `AsyncIterator`). Non-streaming handlers get a runtime-derived request model with `stream` removed via `create_model`, so clients cannot send `stream: true` to endpoints that ignore it. Streaming bypasses the queue; what gets streamed depends on what the function returns:
